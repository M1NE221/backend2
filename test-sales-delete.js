...
function validateSaleResponse(response, context) {
  const { data } = response;
  if (!data?.success || !data?.data?.sale) {
    console.log(`❌ Respuesta inválida en ${context}:`, data);
    return false;
  }
  return true;
}
...
async function testSalesAPI() {
  ...
  try {
    ...
    if (!validateSaleResponse(saleResponse, 'GET /api/sales/:id')) return;
  ...
  try {
    ...
    if (!validateSaleResponse(editResponse, 'PUT /api/sales/:id')) return;
  ...
  try {
    ...
    if (!validateSaleResponse(editWithNotesResponse, 'PUT /api/sales/:id (notas)')) return;
  ...
  try {
    ...
    console.log('\n6️⃣ Eliminando venta...');
    const deleteResponse = await axios.delete(`${BASE_URL}/api/sales/${TEST_CONFIG.saleId}`, { headers });
    
    console.log('✅ DELETE /api/sales/:id - Status:', deleteResponse.status);
    console.log('🗑️ Venta eliminada:', deleteResponse.data);

  } catch (error) {
    console.log('❌ Error eliminando venta:', error.response?.data || error.message);
  }

  try {
    // PRUEBA 7: Intentar obtener venta eliminada
    console.log('\n7️⃣ Verificando que la venta fue eliminada...');
    await axios.get(`${BASE_URL}/api/sales/${TEST_CONFIG.saleId}`, { headers });
    
    console.log('❌ ERROR: La venta debería haber sido eliminada');

  } catch (error) {
    if (error.response?.status === 404) {
      console.log('✅ Venta eliminada correctamente - no se encuentra');
    } else {
      console.log('❌ Error inesperado:', error.response?.data || error.message);
    }
  }
